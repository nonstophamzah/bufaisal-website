...
- **Hosting:** Vercel
- **Analytics:** Facebook Pixel, Google Analytics, Microsoft Clarity (session recording, public routes only — see the Clarity convention below before touching it or the CSP).
...
- **Microsoft Clarity is a two-stage loader — do not "simplify" it (locked 2026-08-17, commit `a2fbd86`):** [`src/components/MicrosoftClarity.tsx`](src/components/MicrosoftClarity.tsx) MUST render Clarity's **inline bootstrap stub**, never a bare `<Script src="https://www.clarity.ms/tag/<id>" />`. The tag at `/tag/<id>` is a 707-byte loader that does **not** define `window.clarity` — its first statement *calls* it (`a[c]("metadata", ...)` with `a=window, c="clarity"`) and it reads `a[c].q` / `.v` / `.t`. Without the stub it throws `TypeError: a[c] is not a function` on line 1 and Clarity silently never starts: no `window.clarity`, no `_clck`/`_clsk` cookies, nothing uploaded, **zero sessions in the dashboard with no visible error on the page**. Seeing two `script[src*=clarity]` tags in the DOM is CORRECT and is not a double-mount — the stub injects the tag, the tag injects the real library from `scripts.clarity.ms`. Exactly one Clarity script is authored by us (one `<MicrosoftClarity />` mount in `layout.tsx`, one `clarity.ms/tag` string in `src/`); the tag carries its own dup-guard. Route gating (the internal-route prefix list) and the hardcoded `xrxwld2ts1` fallback are deliberate — there is no `NEXT_PUBLIC_CLARITY_PROJECT_ID` in Vercel and none is needed.
- **Clarity CSP entries must stay wildcarded, never pinned (locked 2026-08-17, commit `a2fbd86`):** all three directives in [`next.config.mjs`](next.config.mjs) use `https://*.clarity.ms` — `script-src` (tag + `scripts.clarity.ms` library), `connect-src` (session upload), `img-src` (`c.clarity.ms/c.gif` ID-sync pixel). **The collect shard rotates:** `n.clarity.ms`, `t.clarity.ms` and `a.clarity.ms` were all observed within a single debugging session. Pinning specific hosts will break ingest intermittently and look like a Clarity outage rather than a CSP bug. Derive any future host changes by reading the live tag source (`curl https://www.clarity.ms/tag/xrxwld2ts1`), not from documentation — the docs-sourced `c.bing.com` entry was wrong, appeared in neither the tag nor the 74 KB library, and was removed. Clarity failures are CSP-shaped and silent: the tag lands in the DOM and the console CSP error is the only signal, so a raw `curl` of the page HTML proves nothing (the script is `lazyOnload`, injected client-side). Verify in a real browser: `typeof window.clarity === 'function'`, `_clck`/`_clsk` cookies set, and a `*.clarity.ms/collect` request returning **204** (not 200 — 204 No Content is the success status for the ingest beacon).
- **`PageViewTracker` is the ONLY source of pageviews, on both platforms (locked 2026-08-17, commits `a13631c` + `bb910f5`):** [`src/components/PageViewTracker.tsx`](src/components/PageViewTracker.tsx) fires both `fbq('track','PageView')` and `gtag('event','page_view')`. The GA4 config runs `send_page_view: false` and the Meta bootstrap in `layout.tsx` calls only `fbq('init')` — **do not re-add a script-level PageView to either.** They previously fired one each AND the tracker fired another, double-counting every real load on every route (2026-08-16: `/item/*` 2.42 views/user, `/categories` 3.15, against ~1.2 and ~1.6 real loads — the apparent "baseline" was itself doubled).
  - **Key on the semantic view, never on the `useSearchParams()` object.** Its identity changes on every navigation, so keying on it counted each infinite-scroll step as a pageview: `loadMore` does `router.replace('?page=N')` per step (`/` 10.49 views/user, `/shop` 13.07 vs ~2.4 on non-scrolling routes; Meta optimised delivery against ~4x real traffic). Deps are `pathname + category + q`. `page` (scroll depth) and `redirectedFrom` (display-only label whose dismiss also does a `router.replace`) are excluded, and `page` is kept out of the reported `page_search` too.
  - **Switching `loadMore` to `history.replaceState` does NOT avoid this.** Next intercepts it and syncs `useSearchParams()`, so the effect fires anyway — verified: a bare `replaceState` with no router involvement produced 1 gtag `page_view` + 1 fbq `PageView`.
  - **Dispatch on analytics-ready, not on effect-run.** Both bootstraps are `afterInteractive`, so neither global reliably exists when the effect runs, and both call sites are `if`-guarded — a missing global silently drops the event. On live `/item/[id]`: `domInteractive` 168ms but `fbevents.js` didn't start until 266ms, losing the Meta PageView on every item-page load. `whenReady()` polls for each global independently and cancels on unmount. GA4 only won that race by sitting earlier in `<head>`.
  - **Verifying:** count `fbq`/`gtag` **calls**, not network beacons. Meta uses `navigator.sendBeacon` on some pages, which never appears in Resource Timing — that reads as a false zero. Dev also double-counts via React StrictMode's effect double-invoke; verify against a production build.
- **Never set `scroll-behavior: smooth` on `html` (locked 2026-08-17, commit `c557ec1`):** it silently defeats back-navigation scroll restoration on every feed — the browser's restore scroll is animated rather than applied and gets lost. A/B on `/?page=3` (scroll to 9000 → open an item → Back): `smooth` → `scrollY 0`, `auto` → `scrollY 9000`, with identical document height and card count (18339px / 150 items) both times. This is what made back-nav feel broken; the cumulative `?page=N` refetch was reconstructing the feed correctly all along (resolves the item left unverified in the 2026-06-18 handoff). Scope smooth scrolling to the specific element, or pass `behavior: 'smooth'` to `scrollIntoView()`/`scrollTo()`, which ignores this CSS property.
- **`/shop` canonicals are built by WHITELISTING params (locked 2026-08-17, commit `6e84f35`):** `buildShopCanonical()` in [`src/app/shop/page.tsx`](src/app/shop/page.tsx) admits only `category` (when it resolves to a known slug) and `q`. Never add a param to the canonical by "stripping the bad ones" — a param added later would leak into the index by default. Current matrix: bare → `/shop`; known slug → **self**; known slug with **0** visible items → self + `noindex, follow`; legacy alias → the **resolved** slug (consolidates old/new URLs); unknown slug → `/shop` (it renders the unfiltered feed, so self-canonicalising it would open an unbounded duplicate space off one typo'd inbound link); `?q=` → **self** + `noindex, follow`; `?page=N` → the page-1 URL of that filter; `redirectedFrom` → stripped. `og:url` is derived from the same value — before this commit they disagreed, and every variant hardcoded `canonical: '/shop'`, telling Google all 11 category pages were duplicates of the unfiltered feed (contradicting locked Architecture decision **2.4**, "Filter URLs are SEO-indexable… canonical, indexable", and undermining PR #54's SSR ItemList, since signals on a page treated as a duplicate are attributed to the canonical target).
  - **Search pages must be SELF-canonical, never pointed at `/shop`.** Google warns that `noindex` plus a cross-URL canonical can propagate the noindex to the target; canonicalising `?q=` to `/shop` risks deindexing `/shop` itself. This is the one place the intuitive fix is the wrong one.
  - **The empty-category threshold is exactly 0, never a "thin" threshold.** Inventory turns over daily; a `<=2` rule would flap categories in and out of the index. Thin-but-real pages simply don't rank, which costs nothing. The count query **fails open** (returns 1 on error) — wrongly noindexing a stocked category is far worse than briefly indexing an empty one.
  - **`page` is dropped from the canonical** because page N is a cumulative superset of page 1, not distinct content, and it mutates the emitted ItemList (`numberOfItems` 50 at `page=1` vs 83 at `page=2` for the same category). Safe **only** because every item URL is in `sitemap.ts`, so crawl depth doesn't carry item discovery — re-check this if that ever stops being true.
  - **`sitemap.ts` lists only canonical indexable URLs.** The 11 category slugs are included with `lastModified` = freshest `published_at` in that category; aliases, 0-item categories, and every `?q=`/`?page=`/`?redirectedFrom=` URL are excluded. Sitemap inclusion must never land *before* the canonical fix — listing URLs that declare themselves duplicates reinforces the wrong signal.
  - **Not URL-addressable, so invisible to Google:** `sortBy` and `activeSubcategory` in `shop-client.tsx` are local `useState`. The subcategory tabs (Beds / Wardrobes / Nightstands…) generate no URL at all, so the multi-axis filter long-tail decision 2.4 actually describes (Type, Brand, Price, Condition, Shop) does not exist yet — `?category=` is one axis of five.
- **Feed queries select `FEED_COLUMNS`, never `select('*')` (locked 2026-08-17, commit `a0dbbc0`):** [`src/lib/feed-query.ts`](src/lib/feed-query.ts) defines the 27 columns the public feed reads, consumed by **all four** fetch sites so they can't drift — `getItems()` in `page.tsx` and `shop/page.tsx`, plus `buildQuery()` and the `subcatPool` fetch in `shop-client.tsx`. `select('*')` shipped all 67 columns including five JSONB schema blobs that only `/item/[id]` reads: ~16 KB of HTML per card, 849 KB on homepage page 1, 5.27 MB at page 7, ~22 MB for a full scroll. Now 303 KB / 1.65 MB (63–69% smaller). **Add new feed columns to the constant, not at the call site.** A missing column degrades to `undefined` at runtime and **TypeScript will not catch it** (rows are cast to `ShopItem`, whose fields are nullable) — the regression test is a byte-identical diff of rendered page-1 HTML (markup + JSON-LD + head) on `/`, `/shop`, and two category URLs.
  - **`ai_barcode_extracted` is load-bearing, not an `ai_*` leftover:** `buildWhatsAppUrl()` puts it in the 🔖 line of the Negotiate pre-fill. Dropping it silently removes the barcode from every WhatsApp message. It is the only `ai_*` column the feed needs. `condition_notes` is genuinely unused by the feed (grepped: `/item/[id]`, `/admin`, AI routes only).
  - **`asShopItems()` exists because supabase-js can only infer a row type from a `select()` it reads as a string literal;** a runtime-built column string types the result as `GenericStringError[]`. The cast is centralised there with its justification rather than repeated at five call sites.
  - **The row-cap guard is `console.error`, deliberately.** The homepage fetches the whole visible catalog every request and PostgREST caps at 1000 rows — past that the feed tail truncates **silently**, items vanishing from the site with no error. `checkFeedRowCap()` fires at ≥900 with the count and route. This is the trigger for **B2** (cursor pagination over a frozen rank window) — see the handoff for the full design; also trigger it if a lean page-7 payload exceeds ~1 MB or a category exceeds ~4 pages.
...
- **Shop feed is URL-driven (locked 2026-06-18):** `ShopClient` (`src/app/shop/shop-client.tsx`, shared by `/shop` and `/`) reads `activeCategory` and the pagination depth `?page=N` from the URL, not local state. SSR `getItems(category, q, page)` in both `shop/page.tsx` and `page.tsx` returns pages `1..N` in one shot via `.range(0, page * SHOP_PAGE_SIZE - 1)` (inclusive). This reconstructs the feed height on back-nav so native scroll restoration can land. `loadMore` is a `router.replace('?page='+next, { scroll: false })` (no history pollution); category/search use `router.push` (each filter change is a back-navigable history entry, and omitting `page` resets depth). A prop-sync effect (`setItems(initialItems)` on `initialItems` change) adopts each SSR payload; the only client-side fetch left is live as-you-type search (`fetchItems`, fires on a non-empty `search` term only — empty falls through to the SSR list). Don't reintroduce client-side pagination append or `setActiveCategory` — both were removed in this cutover.
- **Category sort-priority (locked 2026-06-21):** category feeds pin high-intent items to the top so shoppers don't have to scroll past low-intent ones (sofas were buried under TV stands, beds under wardrobes). Rules live in `src/lib/category-sort.ts` as an ordered `RegExp[]` per canonical `published_category` value — array index = tier (0 = highest); a name matching no rule is "everything else" and sorts last. Current prioritized categories: **Bedroom** (bed → wardrobe → mattress → nightstand/bedside), **Living Room** (sofa/sectional/couch/armchair → coffee table → tv stand/unit), **Dining & Kitchen** (dining table/set → dining chair → kitchen cabinet), **Office & Fitness** (desk → office chair → treadmill/exercise bike → filing cabinet). Appliances, Kids & Baby, Outdoor & Garden, Shoe Racks & Shelves have no rules and stay in recency order. Key invariants: (1) **word-boundary regexes** where a substring would misfire — `\bbed\b` so "Bedside Table" lands in the nightstand tier, not the bed tier; (2) matches the **resolved display name** (`resolvePublicItemFields().itemName`, i.e. `published_item_name ?? item_name`) so legacy rows sort correctly; (3) **stable sort** — within a tier the canonical `is_featured → created_at DESC → id DESC` order is preserved untouched (we deliberately did NOT switch the within-tier order to `published_at`); (4) **whole-category scope** — because Bedroom (130) and Living Room (60) exceed `SHOP_PAGE_SIZE` (50), `getItems` skips the DB `.range()` for a prioritized category, fetches the full category (≤ a few hundred rows), priority-sorts, then slices to the page window. This keeps the URL-driven pagination contract (pages `1..N`, `hasMore`) identical while ensuring an old-but-high-intent item (e.g. a bed beyond the 50 most-recent) still reaches page 1. Non-prioritized feeds keep the lighter DB-range pagination. Applied identically in both `src/app/page.tsx` and `src/app/shop/page.tsx` (shared `ShopClient`); a category that needs the full fetch is gated by `hasCategoryPriority()`.
...
- **Live category counts (2026-06-21):** inventory has grown well past the 2026-05-12 baseline above. Current visible counts for the prioritized categories: **Bedroom 130, Living Room 60, Dining & Kitchen 19, Office & Fitness 17.** Bedroom and Living Room both exceed `SHOP_PAGE_SIZE` (50) — see the category sort-priority convention below for why that matters.
- **Category taxonomy rename (2026-06-21, migration 023):** 5 of the 8 categories were renamed in both display + data. The category string is the canonical `published_category` value AND the AI's output value, so this was a coordinated DB-migration-first change (migration `023_rename-categories.sql`, run in prod) followed by the code/prompt cutover. Renames: `Bedroom & Sleep → Bedroom`, `Living Room & Lounge → Living Room`, `Kitchen & Dining → Dining & Kitchen`, `Office, Study & Fitness → Office & Fitness`, `Everyday Essentials → Shoe Racks & Shelves` (Appliances, Kids & Baby, Outdoor & Garden unchanged). Slugs renamed to match (`bedroom`, `living-room`, `dining-kitchen`, `office-fitness`, `shoe-racks-shelves`); **old slugs kept as backward-compat aliases** via `CATEGORY_SLUG_ALIASES` + `resolveCategorySlug()` in `constants.ts` — every URL category lookup (SSR `getItems`, `generateMetadata`, `shop-client` active-pill/intro) normalizes through it so indexed `?category=bedroom-sleep` links still resolve. The locked SEO Agent v1.0 prompt taxonomy + decision tree were updated so new AI items emit the new names. NOTE: `Everyday Essentials → Shoe Racks & Shelves` is a label change on what is still the **catch-all default** category (lamps/decor/storage/other map there); the prompt's Step-8 default and product_type vocab keep those items but under the new name.
...
Full brief: [`.claude/handoffs/2026-08-17-fix2-pageview-inflation-start.md`](.claude/handoffs/2026-08-17-fix2-pageview-inflation-start.md).
Four commits direct-to-`main` (`c557ec1`, `a13631c`, `e5a0e41`, `bb910f5`), all
deployed and verified in production.

The GA4/Meta inflation was **not** caused by the cumulative refetch, as originally
assumed — it was `PageViewTracker` keying on the `useSearchParams()` object, plus a
separate script-level double-fire affecting every route. Both fixed; verified live
at exactly one `page_view` + one `PageView` per real load, **delta 0** across a real
infinite-scroll step (`/shop` → `/shop?page=2`, 50 → 100 items) and **delta 1** on a
category change. Back-nav scroll restoration also fixed — it was one CSS line
(`html { scroll-behavior: smooth }`), not the pagination architecture.

**Not started: Defect B** (the cumulative refetch, ~6 MB / 5.68s per scroll step —
real, but invisible to GA4). Recommendation remains **approach B**, not C; the
handoff explains why the stop condition that would have flipped it was falsified,
and records the drift constraint any append-based design must satisfy.

---
SESSION: August 5, 2026 — Site branding + homepage daily shuffle

SHIPPED (all direct-to-`main`, not yet pushed at time of writing — awaiting approval):
- **Part 1 — Favicon set.** Replaced the old default `src/app/favicon.ico` with a
  Bufaisal "BF" monogram (black on yellow #F9D923). Full app-router set:
  `src/app/favicon.ico` (16/32/48), `src/app/icon.png` (512), `src/app/apple-icon.png`
  (180), plus `public/icon-192.png` + `public/icon-512.png` wired through new
  `src/app/manifest.ts` (`/manifest.webmanifest`). All generated by
  `scripts/generate-branding.mjs` (deterministic sharp-based generator, safe to re-run —
  edit the SVGs there and re-run to tweak the mark). Org JSON-LD `logo` in `layout.tsx`
  repointed from the wide `og-image.png` to the square `/icon-512.png`.
- **Part 2 — OG / share images.** Root cause of "share shows latest item's photo": home +
  shop redefined `openGraph` WITHOUT `images`; Next merges `openGraph` shallowly, so the
  layout default was dropped and WhatsApp scraped the first product `<img>`. Fix: created a
  branded yellow 1200×630 card `public/og-default.png` ("BU FAISAL / UAE's Largest Used
  Goods Market — Since 2009") and set `openGraph.images` + `twitter.images` explicitly on
  `layout.tsx` (default), `page.tsx` (home), `shop/page.tsx` (both branches), and
  `categories/page.tsx`. Product pages (`item/[id]`) unchanged — verified they still emit
  the per-product Cloudinary photo as og:image. Old `public/og-image.png` (black card) left
  in place but no longer referenced.
- **Part 3 — Homepage feed: hybrid daily-seeded shuffle** (SUPERSEDES the June-22
  "DO NOT TOUCH interleaveByCategory()" note — Hamzah authorized the change). `interleaveByCategory()`
  in `page.tsx` still round-robins across categories in FEED_PRIORITY order (anti-flood,
  unchanged), but WITHIN each category bucket the order is now a deterministic daily shuffle
  instead of newest-first, so old inventory surfaces instead of staying buried. Seed =
  `YYYYMMDD` in Asia/Dubai (fixed UTC+4); both seed and the "fresh" window flip only at
  Dubai midnight, so the whole-feed order is stable all day → the slice-based `?page=N`
  pagination stays dup/skip-free (verified live: page2 = exact page1 prefix, 0 dups). Mild
  recency boost: items created in the last ~24–48h get their shuffle key multiplied by
  `FRESH_COMPRESSION = 0.35` (compressed toward the front of their bucket, NOT pinned — no
  solid newest-first block; measured ~2.3× top-50 over-representation, longest fresh run at
  top = 0). New pure helpers `hash01()` (FNV-1a + avalanche) and `dailyFeedContext()`.
  "Just Arrived" badge logic untouched. `/shop` + category pages untouched (still recency/priority).

VERIFIED: tsc clean on all touched files (the 1 remaining project error is the pre-existing
untracked `src/scripts/audit-cloudinary-orphans.ts`, not mine); all 6 icon/OG assets serve
HTTP 200; meta tags correct on /, /shop, /categories, /item/[id]; homepage renders with
category balance + Just Arrived badges; no browser console errors. The `>2MB Next data cache`
warning in dev logs is PRE-EXISTING (bare-homepage full-catalog fetch, ~7MB / 500+ rows —
approaching the 1000-row PostgREST cap the June-22 handoff flagged), not a regression.

NOTE for future sessions: the "DO NOT TOUCH: interleaveByCategory()" line in the June-22
handoff block below is now OBSOLETE — the function was intentionally rewritten this session.
...
- **2026-06-20** — Search UX hardening + synonyms + cover-photo data audit. Full brief: [`.claude/handoffs/2026-06-20-search-flash-synonyms-cover-audit-start.md`](.claude/handoffs/2026-06-20-search-flash-synonyms-cover-audit-start.md). All direct-to-`main`, HEAD `385eca4`, clean.
  - **Category redirect now uses `window.location.href`** (full reload) — `ddaccf0`, after 5 commits failed to fix a same-route stale-result flash (root cause: flushSync commits `loading=true` but can't force a browser paint before a fast SSR response replaces it). Don't revert to `router.push`. Redirect is submit-only (`a5c08d2`) and carries only `category`+`redirectedFrom`, never `?q` (`385eca4`).
  - **Search synonym map** (`0b4e183`, `src/lib/search-synonyms.ts`): query-only canonicalization, data-driven. Catalog uses `ac`/`tv` (long forms `air conditioner`/`television` return 0 rows), so synonyms map TO the short forms. `fridge→refrigerator`, `washer→washing machine`, `couch→sofa`, `closet→wardrobe`, etc.
  - **Appliances category triggers reduced to `['appliances','appliance']`** (`bf4ce33`, `290c6dd`) — all specific product terms fall through to keyword search so shoppers see just those products, not the 22-item category.
  - **Cover-photo data fix (Hamzah-run SQL, no app code):** `worker_photo_brand_url` is a misnomer — slot 1 was relabeled Brand→Full Item on 2026-06-15 (`ffb0f59`) without migrating the column, so pre-cutover rows had the brand plate as the cover. 48 visible old rows backfilled `thumbnail_url`/`image_urls[1]` = `worker_photo_2_url`. Do NOT rename the column.
- **2026-06-18** — Shop search-redirect + back-nav scroll fix. Full brief: [`.claude/handoffs/2026-06-18-shop-search-and-back-nav-start.md`](.claude/handoffs/2026-06-18-shop-search-and-back-nav-start.md). All committed directly to `main` (no PRs), HEAD `08eea5e`, clean.
  - **Category-name search redirect** hardened: synonym map expanded (`376ecec`), keyword-vs-redirect race fixed (`fe5a65e`), and `detectCategorySlug`/`CATEGORY_SEARCH_TRIGGERS` extracted to `src/lib/category-search.ts` so the **Navbar** search now routes through it too (`a8593d1`) — the Navbar previously bypassed redirect detection entirely.
  - **"Showing X for 'term'" redirect label** (`edf1ead` + `cd5d4c6`): matched term carried as a display-only `redirectedFrom` URL param (never feeds the filter); search box cleared on redirect; label dismissible. Read the handoff — Hamzah reversed the "keep term in box" approach twice; the final state is the label.
  - **Back-button scroll fix** (`08eea5e`): shop feed is now **URL-driven** — `?page=N` depth, SSR `getItems(category, q, page)` returns pages 1..N in one shot, `loadMore` = `router.replace('?page='+next, {scroll:false})`, category/search use `router.push`, `activeCategory`/`pageNum` read from URL (removed `setActiveCategory`). Two items still need real-browser verification (scroll-restoration timing under `force-dynamic`; deep-page payload size) — see handoff.
...
- **2026-06-15** — Admin dashboard (`/admin`) improvements, [PR #64](https://github.com/nonstophamzah/bufaisal-website/pull/64) merged. Migration 022 (`supabase/migrations/022_add-sold-channel.sql`) is **live in production**.
  - `shop_items.sold_channel` column added: TEXT, nullable, `'online'`/`'shop'` (CHECK-constrained, NULL on rows sold before the column). Added to the `ShopItem` interface.
  - "Mark as Sold" split into **Sold Online** / **Sold in Shop** (bulk toolbar + per-row). Surgical new actions `mark_sold_online` / `mark_sold_shop` in `/api/admin/items` + `/api/admin/items/batch` set `is_sold=true` + `sold_channel`. Sold-tab badge: Online=blue, In Shop=green.
  - Admin item list has a client-side **search bar** (barcode OR item name, case-insensitive); select-all scoped to the filtered view.
  - `created_at` date (`DD MMM YYYY`) now shown on every admin item row.
  - Dependency-free **photo lightbox** on row thumbnails (Escape/backdrop/✕ close, arrow-key nav over existing photo URLs).
  - `ALLOWED_DELETERS` in `/api/admin/items/batch/route.ts` now includes `'Admin'` (the only real `ADMIN_PIN_HASHES` names are `Admin` + `Humaan`; the other allowlist entries don't match any PIN).
