...
└── lib/
    ...
    ├── resolve-public-item-fields.ts  # Phase 6.3 resolver: the seven text fields the public site reads off shop_items. Pattern `published_X ?? legacy_X` (?? not ||). Phase 6.4 PR A extended it with the five JSONB fields (productSchema, faqSchema, specTable, faqs, trustSignals). Retires in Phase 6.5 once legacy columns are dropped.
...
## Listing Generator Pipeline (Phases 1–6.4 complete and verified, May 2026)
...
- **Negotiable source (locked Phase 6.4 PR A):** read `item.admin_negotiable ?? item.worker_negotiable` for public-site display + schema. The legacy `item.negotiable` mirror retires in Phase 6.5; do not introduce new readers against it.
...
- **Tiered similar-items query pattern (locked Phase 6.4 PR B):** `src/lib/similar-items.ts` runs Tier 1 (same brand + same category) → Tier 2 (same category + same shop) → Tier 3 (same category overall). Within each tier order by `published_at DESC` (freshness). Dedupe across tiers by id. Target = 8, MIN_THRESHOLD = 4 → return `[]` (hide section) when fewer than 4 matches. Filter columns are `published_brand` / `published_category` for Phase 6.5 forward-compat.
...
## Phase 6 cutover snapshot (as of 2026-05-12)

### ShopItem interface fields added during Phases 6.3 + 6.4
...
Google Rich Results Test on `/item/4cea5546-1da6-48cb-b6c2-bf7a61232278` returns **5 valid schema items**: Product snippets, Merchant listings, Breadcrumbs, FAQ, Organization. Open non-critical warnings (deferred — out of Phase 6.4 scope):

- **Product snippets / Merchant listings — missing `review` + `aggregateRating`.** Will be addressed via Organization-level `aggregateRating` pulled from Google Business Profile in a future PR. Store-level reviews are the correct pattern for used-goods marketplaces (per-product reviews don't scale and don't match Bufaisal's catalog turnover).
- **Merchant listings — missing `shippingDetails` + `hasMerchantReturnPolicy`.** These unlock Google Shopping eligibility. Future PR.
- **Organization — missing `postalCode`, `streetAddress`.** Future Organization-block upgrade PR.

These warnings are forward-reference notes, not Phase 6.5 work.
...
- **2026-05-12** — Phase 6.4 PR A (`/item/[id]` schema + data wiring, PR #44 at commit `495e0b7`) and Phase 6.4 PR B (layout & conversion polish, PR #45 at commit `3e24686`) both shipped to main. Public `/item/[id]` is now fully cut over to `published_*` columns for text + JSONB; render-time SEO augmentation lives in `src/lib/augment-product-schema.ts`; tiered similar-items query lives in `src/lib/similar-items.ts`; shop GBP config lives in `src/lib/shops.ts`. `yet-another-react-lightbox@^3` added as the repo's first image-display library.
- Next session starts **Phase 6.5** — retire the legacy mirror block in `src/lib/admin-pending-publish.ts` lines ~116–136 and retire `resolvePublicItemFields()` after legacy columns are dropped from `shop_items`. See [`.claude/handoffs/2026-05-12-phase-6.5-start.md`](.claude/handoffs/2026-05-12-phase-6.5-start.md).
