...
│   ├── admin/              # Legacy admin dashboard (settings, analytics, sold/hidden/live tabs, and the legacy Pending tab). Untouched by Phase 5 except a small "→ New Pending" BETA link added to the nav. Approve here flips status='pending' → 'published' and sets is_published=true (PR #24 hotfix; full publish_* compute happens in Phase 5's sidecar).
