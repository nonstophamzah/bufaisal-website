...
import { checkManagerCode } from '@/lib/carpenter-tracker-api';

const PIN_LENGTH = 4;

export default function CarpenterManagerGatePage() {
  ...
  const handleSubmit = useCallback(async (pin: string) => {
    ...
    if (match) {
      sessionStorage.setItem('carpenter_manager', 'ok');
      setSuccess(true);
      setTimeout(() => router.push('/carpenter-tracker/manager'), 600);
    } else {
      setError(true);
  ...
  return (
    <div className="relative flex-1 flex flex-col items-center justify-center px-6 min-h-[calc(100vh-56px)] bg-[#111] pb-16">
      ...
      <div className="grid grid-cols-3 gap-3 w-full max-w-[280px]">
        ...
        <button
          onClick={() => router.push('/carpenter-tracker')}
