...
import { checkEntryCode } from '@/lib/carpenter-tracker-api';

const PIN_LENGTH = 4;

export default function CarpenterTrackerCodePage() {
  ...
  const handleSubmit = useCallback(async (pin: string) => {
    ...
    if (match) {
      sessionStorage.setItem('carpenter_code', 'ok');
      setSuccess(true);
  ...
  if (authenticated) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center px-6 min-h-[calc(100vh-56px)] bg-[#111] gap-5 pb-16">
        ...
        <button
          onClick={() => router.push('/carpenter-tracker/select')}
        ...
        >
          ...
          <span className="font-heading text-3xl">CARPENTERS</span>
          <span className="text-sm opacity-60">Log items</span>
        </button>

        <button
          onClick={() => router.push('/carpenter-tracker/manager-gate')}
        ...
        >
          ...
          <span className="text-sm text-gray-400">Daily totals & rates</span>
        </button>
      </div>
    );
  }
  ...
  return (
    <div className="relative flex-1 flex flex-col items-center justify-center px-6 min-h-[calc(100vh-56px)] bg-[#111] pb-16">
      ...
      {error && <p className="text-red-400 font-bold text-sm mb-4">Wrong code</p>}
      {loading && <Loader2 size={24} className="animate-spin text-yellow mb-4" />}
      {success && <p className="text-green-400 font-bold text-sm mb-4">Access granted</p>}
